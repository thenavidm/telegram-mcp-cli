/**
 * COPY THIS FILE. Do not rewrite it from the reference.
 *
 * Replace Telegram, telegram and TELEGRAM throughout, then check three
 * things against the repo you are adding it to:
 *
 *   1. `TelegramError` is whatever your api/errors.ts exports.
 *   2. `TelegramApi` is whatever api/client.ts exports.
 *   3. Your tools/kit.ts exports `makeContext`, `AnyToolSpec` and `ToolContext`,
 *      and tools/index.ts exports `ALL_TOOLS`. If it does not, the repo has no
 *      seam yet and that comes first: see references/cli-surface.md.
 *
 * Everything else is generic. The parsing, the flag derivation, the exit codes
 * and the guard wiring are the same in every repo, which is the point.
 */
 ...
 *
 * The package ships two binaries onto the same file. Printing `telegram-mcp` at
 * someone who typed `telegram-cli` hands them a command that works but is not
 * the one they have in their fingers.
 */
function binName(): string {
  const name = (process.argv[1] ?? "").split("/").pop() ?? "";
  return name.startsWith("telegram-cli") ? "telegram-cli" : "telegram-mcp";
}
...
export async function runCli(argv: string[]): Promise<number> {
  ...
  if (!spec) {
    ...
    emitError(
      new Error(
        hidden
          ? `${name} is unavailable: this server is running with TELEGRAM_READ_ONLY=1.`
